...
`Interface` has two associated types (`Parameters` and `Response`), and `APIClient.send`/`DataTaskProvider.dataTask` return `T.Response`. `any Interface` cannot be used as a dispatch type - a heterogeneous collection of Interfaces (for example `[any Interface]`, for a request queue) does not type-check, because the associated types make the protocol non-existential for that purpose.
